...
class AuthenticationServiceImplTest {
    ...
    void register_ok() {
        ...
        Assertions.assertNotNull(actual, "Can't return null");
        Assertions.assertEquals(TEST_EMAIL, actual.getEmail(),
                String.format("Expect user with email: %s, but was: %S",
    ...
    void login_withIncorrectPassword_noOk() {
        Mockito.when(userService.findByEmail(TEST_EMAIL)).thenReturn(Optional.of(testUser));
        Mockito.when(passwordEncoder.matches(TEST_PASSWORD, "wrongEmail")).thenReturn(false);
        Assertions.assertThrows(AuthenticationException.class, () -> {
            authenticationService.login(TEST_EMAIL, TEST_PASSWORD);
        });
    }
